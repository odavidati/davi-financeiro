...
function QuickAddSheet({ month, onClose, dispatch }) {
  const [form, setForm] = useState({ description: '', amount: '', category: 'outros', type: 'expense' })
  ...
  function submit() {
    ...
    dispatch({
      type: 'ADD_TRANSACTION', month,
      tx: { id: uid(), date: new Date().toISOString().slice(0,10), description: form.description.trim(), amount: form.type === 'expense' ? -amt : amt, category: form.category, type: 'manual' }
    })
    onClose()
  }
  ...
  return (
    <div className="overlay-backdrop" onClick={onClose}>
      <div className="sheet" onClick={e => e.stopPropagation()}>
        ...
        <button className="btn btn-primary btn-full" style={{ fontSize: 15, padding: '15px' }} onClick={submit}>
          Lançar agora
        </button>
      </div>
    </div>
...
function AppShell({ user, signOut }) {
  ...
  return (
    <div className="app">
      ...
      {tab === 'home'    && <Home onImpulse={() => setImpulse(true)} onGoToContas={() => setTab('contas')} />}
      {tab === 'contas'  && <Contas />}
      {tab === 'gastos'  && <Gastos />}
      {tab === 'analise' && <Analise />}
      {tab === 'metas'   && <Metas />}
      {tab === 'config'  && <Config onSignOut={signOut} />}

      {/* FAB — visible on home and gastos */}
      {(tab === 'home' || tab === 'gastos') && <FAB onClick={() => setQuick(true)} />}
